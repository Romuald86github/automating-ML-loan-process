...
def run_streamlit_app():
    ...
    for feature_name in selected_feature_names:
        if feature_name in ['loan_amount', 'interest_rate', 'term', 'credit_score', 'annual_income']:
            ...
            input_data[feature_name] = st.selectbox(f"{feature_name.replace('_', ' ').capitalize()}", options=preprocessing_pipeline.named_transformers_['categorical_encoder'].transformers_[0][1].categories_[selected_feature_names.index(feature_name)])
